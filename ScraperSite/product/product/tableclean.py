...
def create_csv(table):
    headers = ['Brand', 'Description', 'Link', 'List Price ($)', 'Product Name',
               '# Reviews', 'Price ($)', 'Rank', 'Rating']
    rows = []
    for i in range(len(table[0])):
        new_row = []
    ...
    cleaned_table = cleaned_table[['Rank', 'Product Name', 'Brand', 'Price ($)',
                                   'Rating', '# Reviews', 'Link', 'Description',
                                   'List Price ($)']]
    cleaned_table.to_csv("Cleaned_{}.csv".format('ScrapedData'), index=False)
